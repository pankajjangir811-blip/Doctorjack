...
# =================================================================
# Doctorjack - REQUIREMENTS INSTALLER | DATA-INTEGRITY EDITION v7.7
# Purpose:
#   Installs and verifies prerequisites for doctorjack
#
# Fixes/Improvements:
#   - Forces Go tool PATH priority: ~/go/bin first
#   - Detects/removes conflicting wrong /usr/bin/httpx or Python httpx CLI
#   - Installs ProjectDiscovery httpx, gf, Gxss, qsreplace, uro
#   - Installs GF patterns and verifies gf sqli pattern
#   - Adds strong verification so setup issues are clear before running pipeline
#   - Works for bash and zsh environments
#
# Use only on systems and targets where you have permission.
# =================================================================
...
append_path_to_profile() {
    ...
    touch "$profile"
    cp "$profile" "$profile.bak_doctorjack_$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true

    # Remove old Doctorjack/Go PATH lines to avoid duplicate or wrong priority
    sed -i '/# Added by Doctorjack installer/d' "$profile" 2>/dev/null || true
    sed -i '/# Doctorjack Go tools/d' "$profile" 2>/dev/null || true
    sed -i '/export GOPATH=\$HOME\/go/d' "$profile" 2>/dev/null || true
    sed -i '/export PATH=\$PATH:\$GOPATH\/bin/d' "$profile" 2>/dev/null || true
    sed -i '/export PATH="\$HOME\/go\/bin:\$PATH"/d' "$profile" 2>/dev/null || true
    sed -i '/export PATH="\$GOPATH\/bin:\$PATH"/d' "$profile" 2>/dev/null || true

    {
        echo ''
        echo '# Doctorjack Go tools'
...
is_projectdiscovery_httpx() {
    local bin_path="${1:-httpx}"

    if ! "$bin_path" -h >/tmp/doctorjack_httpx_help.txt 2>&1; then
        return 1
    fi

    if grep -qiE 'projectdiscovery|httpx is a fast|probe|status-code|tech-detect|follow-redirects' /tmp/doctorjack_httpx_help.txt; then
        return 0
    fi
...
verify_httpx_runtime() {
    ...
    # ProjectDiscovery httpx must support -silent and stdin URL probing.
    if echo "https://example.com" | "$bin_path" -silent -timeout 5 >/tmp/doctorjack_httpx_probe.out 2>/tmp/doctorjack_httpx_probe.err; then
        return 0
    fi
...
echo -e "${CYAN}======================================================"
echo -e "   DOCTORJACK REQUIREMENTS INSTALLER v7.7"
echo -e "======================================================${NC}"

if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
    print_warn "You are running as root. A normal user with sudo is usually better."
fi

PROFILE_FILE="$(detect_profile)"
print_step "Using shell profile: $PROFILE_FILE"

print_step "Updating system and installing base packages"
sudo apt update
sudo apt install -y golang-go git python3 python3-pip curl wget build-essential ca-certificates

print_step "Configuring Go environment with correct PATH priority"
export GOPATH="$HOME/go"
export PATH="$GOPATH/bin:$PATH"
mkdir -p "$GOPATH/bin"

append_path_to_profile "$PROFILE_FILE"
print_ok "Go PATH added to $PROFILE_FILE with ~/go/bin first"

print_step "Checking and removing conflicting httpx tools"

if command -v httpx >/dev/null 2>&1; then
    CURRENT_HTTPX="$(command -v httpx)"
    echo -e "${BLUE}Current httpx:${NC} $CURRENT_HTTPX"
else
    CURRENT_HTTPX=""
fi

# Kali/Debian may have a different /usr/bin/httpx. Remove apt package if present.
if dpkg -s httpx >/dev/null 2>&1; then
    print_step "Removing conflicting apt package: httpx"
    sudo apt remove -y httpx || true
fi

# Python package httpx can create/confuse CLI behavior in some environments.
print_step "Removing Python httpx package if it created a conflicting CLI"
python3 -m pip uninstall -y httpx >/dev/null 2>&1 || true
pip uninstall -y httpx >/dev/null 2>&1 || true

# If /usr/bin/httpx still exists and is not ProjectDiscovery httpx, move it aside.
if [[ -x /usr/bin/httpx ]]; then
    if ! is_projectdiscovery_httpx /usr/bin/httpx; then
        print_warn "/usr/bin/httpx is not ProjectDiscovery httpx. Moving it aside."
        sudo mv /usr/bin/httpx "/usr/bin/httpx_wrong_backup_$(date +%Y%m%d_%H%M%S)" 2>/dev/null || true
    fi
fi
...
print_step "Installing Go-based pipeline tools"
go install github.com/projectdiscovery/httpx/cmd/httpx@latest
go install github.com/tomnomnom/gf@latest
go install github.com/KathanP19/Gxss@latest
go install github.com/tomnomnom/qsreplace@latest

print_step "Installing Python-based URL normalization tool: uro"
if python3 -m pip install --break-system-packages uro >/tmp/doctorjack_pip_uro.log 2>&1; then
    print_ok "uro installed with --break-system-packages"
else
    python3 -m pip install --user uro >/tmp/doctorjack_pip_uro.log 2>&1
    print_ok "uro installed with --user"
fi

print_step "Setting up GF patterns"
mkdir -p "$HOME/.gf"
rm -rf /tmp/doctorjack-gf-repo /tmp/doctorjack-gf-patterns-extra

git clone --depth 1 https://github.com/tomnomnom/gf /tmp/doctorjack-gf-repo >/tmp/doctorjack_gf_clone.log 2>&1
cp /tmp/doctorjack-gf-repo/examples/*.json "$HOME/.gf/" 2>/dev/null || true

git clone --depth 1 https://github.com/1ndianl33t/Gf-Patterns /tmp/doctorjack-gf-patterns-extra >/tmp/doctorjack_gf_patterns_clone.log 2>&1
cp /tmp/doctorjack-gf-patterns-extra/*.json "$HOME/.gf/" 2>/dev/null || true

rm -rf /tmp/doctorjack-gf-repo /tmp/doctorjack-gf-patterns-extra
print_ok "GF patterns installed in $HOME/.gf"

print_step "Refreshing shell command cache"
...
for tool in httpx gf Gxss qsreplace; do
    if command -v "$tool" >/dev/null 2>&1; then
        print_ok "$tool -> $(command -v "$tool")"
    else
        print_err "$tool not found in PATH"
        MISSING=1
    fi
done

if command -v uro >/dev/null 2>&1; then
    print_ok "uro -> $(command -v uro)"
elif python3 -c 'import uro' >/dev/null 2>&1; then
    print_ok "uro Python module is installed"
else
    print_err "uro not found"
    MISSING=1
fi

if [[ -x "$HOME/go/bin/httpx" ]]; then
    print_ok "ProjectDiscovery httpx binary exists at $HOME/go/bin/httpx"
else
    print_err "$HOME/go/bin/httpx missing"
    MISSING=1
fi

if [[ "$(command -v httpx 2>/dev/null || true)" != "$HOME/go/bin/httpx" ]]; then
    print_warn "httpx path is not currently $HOME/go/bin/httpx"
    print_warn "Current path: $(command -v httpx 2>/dev/null || echo 'not found')"
    print_warn "Run: source $PROFILE_FILE"
fi

if is_projectdiscovery_httpx "$HOME/go/bin/httpx"; then
    print_ok "httpx help looks like ProjectDiscovery httpx"
else
    print_err "httpx help does not look like ProjectDiscovery httpx"
    MISSING=1
fi

if verify_httpx_runtime "$HOME/go/bin/httpx"; then
    print_ok "httpx stdin probing works with -silent"
else
    print_err "httpx runtime test failed. Check /tmp/doctorjack_httpx_probe.err"
    MISSING=1
fi

if [[ -f "$HOME/.gf/sqli.json" ]] || ls "$HOME/.gf"/*sqli* >/dev/null 2>&1; then
    print_ok "GF SQLi pattern exists"
else
    print_err "GF SQLi pattern missing in $HOME/.gf"
    MISSING=1
fi

if echo 'https://example.com/index.php?id=1' | "$HOME/go/bin/gf" sqli >/tmp/doctorjack_gf_sqli_test.out 2>/tmp/doctorjack_gf_sqli_test.err; then
    if [[ -s /tmp/doctorjack_gf_sqli_test.out ]]; then
        print_ok "gf sqli test works"
    else
        print_warn "gf sqli command ran but test output was empty"
    fi
else
    print_err "gf sqli test failed. Check /tmp/doctorjack_gf_sqli_test.err"
    MISSING=1
fi


install_doctorjack_command() {
    print_step "Installing global command: doctorjack"

    local tmp_file
    tmp_file="$(mktemp /tmp/doctorjack_main_XXXXXX.sh)"

    cat > "$tmp_file" <<'DOCTORJACK_MAIN_SCRIPT'
#!/usr/bin/env bash
...
DOCTORJACK_MAIN_SCRIPT

    chmod +x "$tmp_file"
    sudo install -m 755 "$tmp_file" /usr/local/bin/doctorjack
    rm -f "$tmp_file"

    if command -v doctorjack >/dev/null 2>&1; then
        print_ok "doctorjack installed at $(command -v doctorjack)"
    elif [[ -x /usr/local/bin/doctorjack ]]; then
        print_ok "doctorjack installed at /usr/local/bin/doctorjack"
        print_warn "/usr/local/bin is not in PATH for this shell. Add it to PATH or run /usr/local/bin/doctorjack"
    else
        print_err "Failed to install /usr/local/bin/doctorjack"
        return 1
    fi

    if doctorjack -h >/tmp/doctorjack_global_help.out 2>/tmp/doctorjack_global_help.err; then
        print_ok "doctorjack command help test passed"
    else
        print_err "doctorjack command help test failed. Check /tmp/doctorjack_global_help.err"
        return 1
    fi
}

if install_doctorjack_command; then
    print_ok "Global Doctorjack command is ready"
else
    MISSING=1
fi

echo -e "\n${WHITE}POST-INSTALL COMMANDS${NC}"
echo -e "${YELLOW}Run this once in the same terminal:${NC}"
echo -e "source $PROFILE_FILE"
echo -e "\n${YELLOW}Verify:${NC}"
echo -e "which httpx"
echo -e "httpx -version"
echo -e "echo \"https://example.com\" | httpx -silent"
echo -e "doctorjack -h"

if [[ "$MISSING" -eq 0 ]]; then
    echo -e "\n${GREEN}INSTALLATION COMPLETE: all required checks passed.${NC}"
    echo -e "${GREEN}Ready to run doctorjack${NC}"
else
    echo -e "\n${RED}INSTALLATION FINISHED WITH WARNINGS/ERRORS.${NC}"
    echo -e "${YELLOW}Fix the items above before using this setup.${NC}"
    exit 1
fi
